---
title: "Introduction"
description: "Overview of the SharaForms REST API, base URL, authentication, and rate limits."
---

Welcome to the **SharaForms REST API** – a straightforward, JWT-based interface that lets you manage workspaces, forms and submissions from your own tools or backend jobs.
...
Tokens can be created in your SharaForms dashboard (see the [API Keys](./api-keys) section). Each token is associated with one or more **abilities / scopes** that restrict what it can do.
...
```bash
curl -H "Authorization: Bearer <access_token>" https://api.sharaforms.com/open/workspaces
```
