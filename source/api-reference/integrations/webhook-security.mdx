...
description: "Learn how to validate webhook signatures from SharaForms to ensure authenticity and integrity"
---

# Validating Webhook Signatures

When you configure a webhook with a signing secret, SharaForms signs each webhook request with an HMAC-SHA256 signature. This allows you to verify that:

1. The webhook came from SharaForms (authenticity)
...
app.post('/webhook', (req, res) => {
  ...
  const secret = process.env.SHARAFORMS_WEBHOOK_SECRET;
...
def webhook():
    ...
    secret = os.environ.get('SHARAFORMS_WEBHOOK_SECRET')
...
Route::post('/webhook', function (Request $request) {
    ...
    $secret = env('SHARAFORMS_WEBHOOK_SECRET');
...
In addition to the signature header, SharaForms will send any custom headers you configured when creating the webhook. These can include authentication tokens, API keys, or other identifiers.
